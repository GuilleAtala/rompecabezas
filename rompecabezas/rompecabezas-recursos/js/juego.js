...
var instrucciones = ["Utiliza las flechas para mover las piezas", "Ordena las piezas hasta formar la siguiente imagen"];
...
function mostrarInstrucciones(instrucciones) {
    for (x in instrucciones) { mostrarInstruccionEnLista(instrucciones[x], "lista-instrucciones") }
...
function cargarUltimoMovimiento(direccion) {
...
function chequearSiGano() {
    ...
    while (cont != 9) {
        for (x in grilla) {
            for (j in grilla[x]) {
                if (grilla[x][j] == grillaOriginal[x][j]) {
                    cont++;
                } else { return false }
            }
        }
...
function mostrarCartelGanador() {
    ...
    document.getElementById('cartelGanador').innerHTML = "FELICIDADES!!! GANASTE!!!! realizaste " + movimientos.length + " movimientos";
}
...
function moverEnDireccion(direccion) {
    ...
    if (posicionValida(nuevaFilaPiezaVacia, nuevaColumnaPiezaVacia)) {
        ...
        cargarUltimoMovimiento(direccion);
    }
}
...
function mostrarJuego() {
    document.getElementById('rompecabezaHtml').style.display = 'block';
    iniciar();
